...
def do_search_atmospheric_min(form):
    """
    Do the search for the "atmospheric_min" output collection by raw SQL query.
    ...
    output_files = write_atmospheric_min(filestem, iso_ids_list, rows, form)
    te = time.time()
    ...
    summary_html = ['<p>Here are the results of the query in "atmospheric_min"'
                    ' format</p>',]
    if settings.LIMIT is not None:
...
def get_fmts(form):
    ...
    for atmospheric_prm in atmospheric_prms:
        prm_field_names.append('%s.val' % atmospheric_prm)
    prm_field_fmts = dict(OutputField.objects.filter(name__in=prm_field_names)
                        .values_list('name', 'cfmt'))
    for atmospheric_prm in atmospheric_prms:
        ocfmts.append('%s' % prm_field_fmts['%s.val' % atmospheric_prm])
...
def write_atmospheric_min(filestem, iso_ids_list, rows, form):
    """
    Write the output transitions file for the "atmospheric_min" output
    collection.
    ...
    nfields = 8 + len(atmospheric_prms)
    source_ids = set()
