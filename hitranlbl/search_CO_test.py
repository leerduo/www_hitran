...
CO_test_prms = ['nu', 'sw', 'a', 'gamma_air', 'n_air', 'gamma_self', 'n_self',
                'delta_air', 'deltap_air', 'delta_self', 'deltap_self',
                'SD_air', 'SD_self', 'y_air', 'y_self']

def do_search_CO_test(form):
    """
    Do the search for the "CO_test" output collection, by raw SQL query.
    ...
    output_files = write_CO_test(filestem, iso_ids_list, rows, form)
    te = time.time()
    ...
    summary_html = ['<p>Here are the results of the query in "CO-test"'
                    ' format</p>',]
    if settings.LIMIT is not None:
...
def get_fmts(form):
    ...
    prm_field_names = []
    for CO_test_prm in CO_test_prms:
        prm_field_names.extend(['%s.val' % CO_test_prm, 
                                '%s.ierr' % CO_test_prm,
                                '%s.ref' % CO_test_prm])
    ...
    for CO_test_prm in CO_test_prms:
        ocfmts.append('%s(%s)[%s]' % (
                                prm_field_fmts['%s.val' % CO_test_prm],
                                prm_field_fmts['%s.ierr' % CO_test_prm],
                                prm_field_fmts['%s.ref' % CO_test_prm]))
        
        # TODO - set Fortran-style specifiers here!
        prm_ffmt = [prm_field_fmts['%s.val' % CO_test_prm], '1X',
                    prm_field_fmts['%s.ierr' % CO_test_prm], '2X',
                    prm_field_fmts['%s.ref' % CO_test_prm], '1X']
...
def write_CO_test(filestem, iso_ids_list, rows, form):
    """
    Write the output transitions file for the "CO_test" output
    collection.
